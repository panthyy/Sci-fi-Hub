const movies = require("./data/wikipedia/movies.json");
const fs = require("fs");
...
fs.writeFileSync(
  path.join(__dirname, "data/wikipedia", "genres.json"),
  JSON.stringify(uniquegenres, null, 2)
);

fs.writeFileSync(
  path.join(__dirname, "data/wikipedia", "actors.json"),
  JSON.stringify(uniqueActors, null, 2)
);

fs.writeFileSync(
  path.join(__dirname, "data/wikipedia", "countries.json"),
  JSON.stringify(uniqueCountries, null, 2)
);

fs.writeFileSync(
  path.join(__dirname, "data/wikipedia", "directors.json"),
